...
def test_get_project_detail(session: requests.Session, project_id: int) -> Dict[str, Any]: 
    try:
...
def test_get_annotation_tasks(session: requests.Session, annotation_project_id: int, limit: int) -> Dict[str, Any]:
    ...
    try:
        ...
        response = session.get(
            ...
            params={
                'limit': limit,
                'offset': 0,
                'annotation_project__eq': annotation_project_id,
            },
            headers={'Content-Type': 'application/json'},
        )
...
def run_tests_project_list(run_number: int = 1, csv_filename: str = 'annotation_projects_time_logs.csv'):
    ...
    try:
        ...
        for project in projects_page['items']:
            t1 = time.time()
            test_get_annotation_tasks(session, project['id'], -1)
            task_time = time.time() - t1
            print(f"Time taken to get annotation tasks for project {project['id']}: {task_time}")
...
def run_tests_project_detail(run_number: int = 1, csv_filename: str = 'annotation_project_detail_time_logs.csv'):
    ...
    try:
        ...
        t0 = time.time()
        # Test 1: Get many projects
        project_detail = test_get_project_detail(session, 78)
        get_many_time = time.time() - t0
        ...
        t1 = time.time()
        test_get_annotation_tasks(session, project_detail['id'], -1)
        task_time = time.time() - t1
        print(f"Time taken to get annotation tasks for project {project_detail['id']}: {task_time}")
...
def run_tests_clips_table(run_number: int = 1, csv_filename: str = 'clips_table_time_logs.csv'):
    ...
    try:
        ...
        t0 = time.time()
        test_get_annotation_tasks(session, 78, 100)
        task_time = time.time() - t0
        print(f"Time taken to get annotation tasks for project 78: {task_time}")
