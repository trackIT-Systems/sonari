...
def test_get_clip_annotations(session: requests.Session, annotation_project_id: int, limit: int) -> Dict[str, Any]:
...
def test_get_annotation_task_detail(session: requests.Session, annotation_task_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        response = session.get(
            f'{BASE_URL}/annotation_tasks/detail/',
            params={
...
def run_tests_annotate_view(run_number: int = 1, csv_filename: str = 'annotate_view_time_logs.csv'):
    ...
    try:
        ...
        annotation_tasks = test_get_annotation_tasks(session, 78, -1)
        ...
        annotation_task_id = annotation_tasks['items'][0]['id'] 
        
        t2 = time.time()
        annotation_task_detail = test_get_annotation_task_detail(session, annotation_task_id)
