...
from sonari import exceptions, models
from sonari.api import common
from sonari.api.io.aoef.features import get_feature_names
from sonari.api.io.aoef.recordings import import_recordings
...
async def import_dataset(
    ...
    if not dataset_dir.is_relative_to(audio_dir):
        raise ValueError(f"Dataset directory {dataset_dir} is not relative " f"to audio directory {audio_dir}")
    ...
    path_mapping = {
        recording.uuid: normalize_path(recording.path, dataset_dir) for recording in dataset_object.recordings or []
    }
