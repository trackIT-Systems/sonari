...
from sonari.api.common import create_objects_without_duplicates
from sonari.api.io.aoef.common import get_mapping
from sonari.api.io.aoef.notes import import_notes
...
async def import_recordings(
    ...
    if not audio_dir.is_relative_to(base_audio_dir):
        raise ValueError(f"Audio directory {audio_dir} is not in the base audio directory")

    # Filter out invalid recordings
    recordings = [rec for rec in recordings if check_recording(rec, audio_dir=audio_dir, base_audio_dir=base_audio_dir)]
    ...
    missing_recordings = [rec for rec in recordings if rec.uuid not in mapping and rec.hash not in mapping]
...
async def _get_existing_recordings_by_uuid(
    ...
    if not uuids:
        return {}
    existing_recordings = await session.execute(select(models.Recording).filter(models.Recording.uuid.in_(uuids)))
    return {rec.uuid: rec.id for rec in existing_recordings.unique().scalars().all()}
...
async def _get_existing_recordings_by_hash(
    ...
    if not hashes:
        return {}
    existing_recordings = await session.execute(select(models.Recording).filter(models.Recording.hash.in_(hashes)))
    return {rec.uuid: rec.id for rec in existing_recordings.unique().scalars().all()}
...
async def _get_existing_recordings_by_path(
    ...
    paths = {normalize_audio_path(rec, audio_dir, base_audio_dir) for rec in recordings}
    if not paths:
        return {}
    existing_recordings = await session.execute(select(models.Recording).filter(models.Recording.path.in_(paths)))
    return {rec.uuid: rec.id for rec in existing_recordings.unique().scalars().all()}
...
async def _create_recording_tags(
    ...
    await create_objects_without_duplicates(
        ...
        key_column=tuple_(models.RecordingTag.recording_id, models.RecordingTag.tag_id),
    )
...
async def _create_recording_notes(
    ...
    notes = [note for recording in recordings for note in recording.notes or []]
...
def check_recording(
    ...
    if not path.is_relative_to(base_audio_dir):
        logger.warning(f"Recording file {path} is not in the base audio directory")
        return False
