...
async def import_tags(
    ...
    stmt = select(models.Tag.id, models.Tag.key, models.Tag.value).where(
        tuple_(models.Tag.key, models.Tag.value).in_({(tag.key, tag.value) for tag in tags})
    )
    ...
    if not missing:
        return {tag.id: mapping[(tag.key, tag.value)] for tag in tags if (tag.key, tag.value) in mapping}

    stmt = insert(models.Tag)
    result = await session.execute(stmt, missing)

    stmt = select(models.Tag.id, models.Tag.key, models.Tag.value).where(
        tuple_(models.Tag.key, models.Tag.value).in_({(tag["key"], tag["value"]) for tag in missing})
    )
    result = await session.execute(stmt)
