...
async def import_annotation_task(
    ...
    if len(annotation_tasks) != len(annotation_project_uuids):
        raise ValueError("The number of annotation tasks and annotation project " "UUIDs must match")
...
async def _create_annotation_tasks(
    ...
    # Get existing
    mapping = await get_mapping(session, {t.uuid for t in annotation_tasks}, models.AnnotationTask)

    missing = [
        (t, proj_uuid)
        for (t, proj_uuid) in zip(annotation_tasks, annotation_project_uuids, strict=False)
    ...
    created = await get_mapping(session, {v["uuid"] for v in values}, models.AnnotationTask)
...
async def _create_annotation_badges(
    ...
    for task in annotation_tasks:
        ...
        for badge in task.status_badges:
            user_db_id = None if badge.owner is None else users.get(badge.owner)
            values.append(
                {
