...
from sonari.api.io.aoef.features import get_feature_names
...
async def get_or_create_evaluation_set(
    ...
    stmt = select(models.EvaluationSet).where(models.EvaluationSet.uuid == obj.uuid)
...
async def add_evaluation_tags(
    ...
    missing = [v for v in values if (v["evaluation_set_id"], v["tag_id"]) not in existing]
    if not missing:
        return
...
async def add_clip_annotations(
    ...
    stmt = select(
        ...
        tuple_(
            models.EvaluationSetAnnotation.evaluation_set_id,
            models.EvaluationSetAnnotation.clip_annotation_id,
        ).in_([(v["evaluation_set_id"], v["clip_annotation_id"]) for v in values])
    )
    result = await session.execute(stmt)
    existing = set(result.all())

    missing = [v for v in values if (v["evaluation_set_id"], v["clip_annotation_id"]) not in existing]
    if not missing:
        return
