...
async def import_sound_event_annotations(
    ...
    if not len(sound_events_annotations) == len(clip_annotation_uuids):
        raise ValueError("The number of sound event annotations and clip annotation UUIDs " "must be equal.")
...
async def _create_sound_event_annotations(
    ...
    missing = [
        (s, ca_uuid)
        for s, ca_uuid in zip(sound_events_annotations, clip_annotation_uuids, strict=False)
    ...
    for annotation, clip_annotation_uuid in missing:
        ...
        created_by_db_id = None if annotation.created_by is None else users.get(annotation.created_by)
...
async def _create_sound_event_notes(
    ...
    notes = [note for annotation in sound_events_annotations for note in annotation.notes or []]
...
async def _create_sound_event_annotation_tags(
    ...
    missing = [v for v in values if (v["sound_event_annotation_id"], v["tag_id"]) not in result.all()]
    if not missing:
        return
