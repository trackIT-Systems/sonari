...
async def import_sound_event_predictions(
    ...
    if not len(sound_events_predictions) == len(clip_prediction_uuids):
        raise ValueError("The number of sound event predictions and clip prediction UUIDs " "must be equal.")
...
async def _create_sound_event_predictions(
    ...
    missing = [
        (s, ca_uuid)
        for s, ca_uuid in zip(sound_events_predictions, clip_prediction_uuids, strict=False)
...
async def _create_sound_event_prediction_tags(
    ...
    missing = [v for v in values if (v["sound_event_prediction_id"], v["tag_id"]) not in existing]
