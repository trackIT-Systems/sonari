...
from sonari.api.io.aoef.features import import_feature_names
...
async def import_sound_event_evaluations(
    ...
    if not len(matches) == len(clip_evaluation_uuids):
        raise ValueError("The number of matches and clip evaluation uuids must be equal.")
...
async def _create_sound_event_evaluations(
    ...
    missing = [
        (match, eval_uuid)
        for match, eval_uuid in zip(matches, clip_evaluation_uuids, strict=False)
    ...
    for match, eval_uuid in missing:
        source_id = None if not match.source else sound_event_predictions.get(match.source)
        target_id = None if not match.target else sound_event_annotations.get(match.target)
...
async def _create_sound_event_evaluation_metrics(
    ...
    stmt = select(
        ...
        tuple_(
            models.SoundEventEvaluationMetric.sound_event_evaluation_id,
            models.SoundEventEvaluationMetric.feature_name_id,
        ).in_((v["sound_event_evaluation_id"], v["feature_name_id"]) for v in values)
    )
    results = await session.execute(stmt)
    existing = {(r[0], r[1]) for r in results.scalars().all()}

    missing = [v for v in values if (v["sound_event_evaluation_id"], v["feature_name_id"]) not in existing]
