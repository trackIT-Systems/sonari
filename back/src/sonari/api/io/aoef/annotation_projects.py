...
from sonari.api.io.aoef.annotation_tasks import get_annotation_tasks
...
async def get_or_create_annotation_project(
    ...
    stmt = select(models.AnnotationProject).where(models.AnnotationProject.uuid == obj.uuid)
...
async def add_annotation_tags(
    ...
    missing = [v for v in values if (v["annotation_project_id"], v["tag_id"]) not in existing]
    if not missing:
        return
