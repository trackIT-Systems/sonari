...
async def find_object(
    ...
    if obj is None:
        raise exceptions.NotFoundError(f"A {model.__name__} with the specified condition was not found")
...
async def create_objects(
    ...
    values = [_add_defaults(value, default_values, default_factories) for value in values]
...
async def remove_tag_from_object(
    ...
    tag = next(
        (tag for tag in getattr(obj, relation_field_name) if tag.tag.id == tag_id),
        None,
    )
...
async def remove_note_from_object(
    ...
    note = next(
        (note for note in getattr(obj, relation_field) if note.note.id == note_id),
        None,
    )
...
def _get_defaults(model: type[A]):
    ...
    for field in fields(model):
        ...
        elif field.default_factory is not MISSING and field.default_factory is not list:
