...
class ClipAPI(
        ...
        for f in obj.features:
            if f.name == feature.name:
                raise exceptions.DuplicateObjectError(
                    f"Clip {obj.id} already has a feature with feature name " f"{feature.name}."
                )
    ...
    async def update_feature(
        ...
        for f in obj.features:
            ...
            raise ValueError(f"Clip {obj} does not have a feature with feature " f"name {feature.name}.")
    ...
    async def _create_clip_features(
        ...
        """
        clip_features = [
            [schemas.Feature(name=name, value=value) for name, value in compute_clip_features(clip).items()]
            for clip in clips
        ]
        ...
        feature_names: dict[str, schemas.FeatureName] = {
            name: await features.get_or_create(session, name=name) for name in names
        }
    ...
    def to_soundevent(self, obj: schemas.Clip, audio_dir: Path | None = None) -> data.Clip:
