...
class SoundEventEvaluationAPI(
    ...
    async def add_metric(
        ...
        for m in obj.metrics:
            if m.name == metric.name:
                raise ValueError(
                    f"Sound event evaluation {obj.id} already has a metric " f"with feature name {metric.name}."
                )
    ...
    async def update_metric(
        ...
        for m in obj.metrics:
            ...
            raise ValueError(
                f"Sound event evaluation {obj.id} does not have a metric " f"with feature name {metric.name}."
            )
        ...
        await update_object(
            ...
            models.SoundEventEvaluationMetric,
            and_(
                models.SoundEventEvaluationMetric.sound_event_evaluation_id == obj.id,
                models.SoundEventEvaluationMetric.feature_name_id == feature_name.id,
            ),
            value=metric.value,
        )
        obj = obj.model_copy(update=dict(metrics=[m if m.name != metric.name else metric for m in obj.metrics]))
        self._update_cache(obj)
        return obj
    ...
    async def remove_metric(
        ...
        obj = obj.model_copy(update=dict(metrics=[m for m in obj.metrics if m.name != metric.name]))
        self._update_cache(obj)
        return obj
    ...
    async def from_soundevent(
        ...
        if data.target:
            sound_event_annotation = await sound_event_annotations.from_soundevent(
                session,
                data.target,
        ...
        if data.source:
            sound_event_prediction = await sound_event_predictions.from_soundevent(
                session,
                data.source,
