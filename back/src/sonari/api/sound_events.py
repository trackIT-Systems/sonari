...
class SoundEventAPI(
    ...
    async def add_feature(
        ...
        for f in obj.features:
            if f.name == feature.name:
                raise ValueError(f"Sound event already has feature {feature.name}.")
    ...
    async def update_feature(
        ...
        for f in obj.features:
            ...
            raise ValueError(f"Sound event does not have feature {feature.name}.")
    ...
    async def create_geometric_features(
        ...
        feature_mapping = {name: await features.get_or_create(session, name=name) for name in feature_names}
    ...
    async def update_geometric_features(
        ...
        sound_event = sound_event.model_copy(
            update=dict(
                features=[
                    (f if f.name not in feature_mapping else feature_mapping[f.name]) for f in sound_event.features
                ]
            )
        )
    ...
    async def get_recording(
        ...
        if db_recording is None:
            raise exceptions.NotFoundError(f"Recording for sound event {sound_event.uuid} not found.")
