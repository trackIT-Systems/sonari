...
class ClipPredictionAPI(
    ...
    async def add_tag(
        ...
        for t in obj.tags:
            if (t.tag.key, t.tag.value) == (tag.key, tag.value):
                raise exceptions.DuplicateObjectError(f"Tag {tag} already exists in clip prediction {obj.id}")
    ...
    async def remove_tag(
        ...
        for t in obj.tags:
            ...
            raise exceptions.NotFoundError(f"Tag {tag} does not exist in clip prediction {obj.id}")
        ...
        obj = obj.model_copy(
            update=dict(
                tags=[t for t in obj.tags if not (t.tag.key == tag.key and t.tag.value == tag.value)],
            )
        )
        self._update_cache(obj)
        return obj
