...
class SoundEventPredictionAPI(
        ...
        for t in obj.tags:
            if (t.tag.key, t.tag.value) == (tag.key, tag.value):
                raise exceptions.DuplicateObjectError(
                    f"Tag {tag} already exists in sound event " f"prediction {obj.id}"
                )
    ...
    async def remove_tag(
        ...
        for t in obj.tags:
            ...
            raise exceptions.NotFoundError(f"Tag {tag} does not exist in sound event " f"prediction {obj.id}")

        await common.delete_object(
            session,
            models.SoundEventPredictionTag,
            and_(
                models.SoundEventPredictionTag.sound_event_prediction_id == obj.id,
                models.SoundEventPredictionTag.tag_id == tag.id,
            ),
        )

        obj = obj.model_copy(
            update=dict(tags=[t for t in obj.tags if t.tag.key != tag.key or t.tag.value != tag.value])
        )
        self._update_cache(obj)
        return obj
    ...
    async def from_soundevent(
        ...
        """Get the Sonari representation of a sound event prediction.
        ...
        -------
        sound_event_prediction : schemas.SoundEventPrediction
            The sound event prediction in Sonari format.
        """
        try:
    ...
    async def _create_from_soundevent(
        ...
        clip_prediction = schemas.ClipPrediction.model_validate(db_clip_prediction)
