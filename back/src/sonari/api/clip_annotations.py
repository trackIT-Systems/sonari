...
class ClipAnnotationAPI(
    ...
    async def add_tag(
        ...
        for t in obj.tags:
            if t.key == tag.key and t.value == tag.value:
                raise exceptions.DuplicateObjectError(f"Tag {tag.id} already exists in clip annotation {obj.id}.")
    ...
    async def add_note(
        ...
        for n in obj.notes:
            if n.id == note.id:
                raise exceptions.DuplicateObjectError(f"Note {note} already exists in clip annotation {obj}.")
    ...
    async def remove_tag(
        ...
        for t in obj.tags:
            ...
            raise exceptions.NotFoundError(f"Tag {tag.id} does not exist in clip annotation {obj.id}.")
        ...
        obj = obj.model_copy(update=dict(tags=[t for t in obj.tags if not (t.key == tag.key and t.value == tag.value)]))
        self._update_cache(obj)
        return obj
    ...
    async def remove_note(
        ...
        for n in obj.notes:
            ...
            raise exceptions.NotFoundError(f"Note {note.id} does not exist in clip annotation {obj.id}.")
