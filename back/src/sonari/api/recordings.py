...
class RecordingAPI(
    ...
    async def create_many(
        ...
        validated_data = remove_duplicates(
            [schemas.RecordingCreate.model_validate(recording) for recording in data],
            key=lambda x: x.path,
        )
    ...
    async def update(
        ...
        if data.time_expansion is not None:
            if data.time_expansion != obj.time_expansion:
                await self.adjust_time_expansion(session, obj, data.time_expansion)
    ...
    async def add_owner(
        ...
        for o in obj.owners:
            if o.id == owner.id:
                raise exceptions.DuplicateObjectError(f"Recording already has an owner with ID {owner.id}")
    ...
    async def remove_owner(
        ...
        for o in obj.owners:
            ...
            raise exceptions.NotFoundError(f"Recording does not have an owner with ID {owner.id}")
        ...
        obj = obj.model_copy(update=dict(owners=[o for o in obj.owners if o != owner]))
        self._update_cache(obj)
        return obj
...
def _assemble_recording_data(
    ...
    if info.hash is None:
        logger.warning(f"Could not compute hash of file. {data.path} Skipping file.")
        return None
