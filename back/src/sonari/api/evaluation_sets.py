...
from sonari.filters.clip_annotations import (
    EvaluationSetFilter as ClipAnnotationEvaluationSetFilter,
)
from sonari.filters.model_runs import (
    EvaluationSetFilter as ModelRunEvaluationSetFilter,
)
from sonari.filters.user_runs import (
...
class EvaluationSetAPI(
    ...
    async def remove_clip_annotation(
        ...
        await delete_object(
            ...
            and_(
                ...
                models.EvaluationSetAnnotation.clip_annotation_id == annotation.id,
            ),
        )
        return obj
    ...
    async def add_tag(
        ...
        if tag in obj.tags:
            raise ValueError(f"Annotation project {obj.id} already has tag {tag.id}.")
    ...
    async def remove_tag(
        ...
        if tag not in obj.tags:
            raise ValueError(f"Annotation project {obj.id} does not have tag {tag.id}.")
        ...
        obj = obj.model_copy(update=dict(tags=[t for t in obj.tags if t.id != tag.id]))
        self._update_cache(obj)
        return obj
    ...
    async def from_soundevent(self, session: AsyncSession, data: data.EvaluationSet) -> schemas.EvaluationSet:
