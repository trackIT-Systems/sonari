...
from sonari.filters.user_runs import UserRunFilter
...
def get_user_runs_router(settings: SonariSettings) -> APIRouter:
    ...
    async def get_user_runs(
        session: Session,
        filter: Annotated[UserRunFilter, Depends(UserRunFilter)],  # type: ignore
        limit: Limit = 100,
        offset: Offset = 0,
