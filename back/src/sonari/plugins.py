...
def load_plugins() -> Generator[tuple[str, ModuleType], None, None]:
    ...
    Will load all modules in the python path that start with 'sonari_'.
    ...
    for _, name, _ in pkgutil.iter_modules():
        if not name.startswith("sonari_"):
            continue
        ...
        if not hasattr(module, "__version__"):
            warnings.warn(f"Plugin {name} has no __version__ attribute.", stacklevel=2)
...
def add_plugin_routes(app: FastAPI, name: str, plugin: ModuleType) -> None:
    ...
    """
    if not hasattr(plugin.module, "router") or not isinstance(plugin.module.router, APIRouter):
        return
